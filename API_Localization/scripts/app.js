...
function displayResult(items) {
    ...
    items.forEach((item, index) => {
        ...
        if(!savedLang)savedLang = getSelectedIds("popup1") || "kor"; 
        const url = `../html/detail.html?id=${encodeURIComponent(item.contentId)}&lang=${encodeURIComponent(savedLang)}`;
...
//현지화
let savedLang = getSelectedIds("popup1") || "kor"; 

async function reloadLocation() {
    savedLang = getSelectedIds("popup1")|| "kor";
